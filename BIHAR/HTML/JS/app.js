...
document.getElementById("chat-icon").onclick = () => {
  ...
  if (currentDisplay === "none") {
    ...
    if (!messages.dataset.greeted) {
      messages.innerHTML += `
        <div class="ai-msg"><b>Saksham:</b> How can I assist you today?</div>
      `;
...
document.getElementById("send-btn").onclick = async () => {
  ...
  messages.innerHTML += `
    <div class="ai-msg" id="${typingId}">
      <b>Saksham:</b> <span class="typing-dots"><span></span><span></span><span></span></span>
    </div>`;
  messages.scrollTop = messages.scrollHeight;

  try {
    // Send to backend with Bihar context
    const response = await fetch("http://localhost:3000/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query: msg, context: "bihar" })
    });
    const data = await response.json();

    // Replace typing indicator with AI reply (Markdown → HTML)
    const typingEl = document.getElementById(typingId);
    if (typingEl) {
      typingEl.outerHTML = `
        <div class="ai-msg"><b>Saksham:</b> ${marked.parse(data.reply)}</div>`;
    }
  } catch (err) {
    const typingEl = document.getElementById(typingId);
